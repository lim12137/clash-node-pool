...
3. **可用性过滤**：用 mihomo 内核对每个节点发起 `generate_204` 探测，延迟 ≤ 5 秒才保留；同时复测上一次订阅里的旧节点，**其中延迟 ≤ 1 秒的继续保留**，与新通过的节点合并去重后按实际延迟升序发布；
...
<!-- STATS:BEGIN -->
**最近一次成功过滤：2026-09-08 07:33（北京时间，GitHub Actions 自动生成）**
...
| **可用节点** | **177（13.3%）** |
| 其中：新通过 / 旧保留(≤1s) | 177 / 0 |
| 最快节点 | 未知 SS-519 | free-nodes（43ms / ss） |
<!-- STATS:END -->
